...
contract TheRewarderChallenge is Test {
    ...
    function getDistributionInfo(string memory _filename, address _participant)
        internal
        view
        returns (uint256 claimAmount, bytes32[] memory proof)
    {
        Reward[] memory rewards =
            abi.decode(vm.parseJson(vm.readFile(string.concat(vm.projectRoot(), _filename))), (Reward[]));
        ...
        for (uint256 i = 0; i < BENEFICIARIES_AMOUNT; i++) {
            ...
            if (rewards[i].beneficiary == _participant) {
                claimAmount = rewards[i].amount;
                participantIndex = i;
        ...
        proof = merkle.getProof(leaves, participantIndex);
        return (claimAmount, proof);
    }
